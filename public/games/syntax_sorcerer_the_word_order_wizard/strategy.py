...
class SyntaxSorcererTheWordOrderWizardStrategy(BaseStrategy):
    _strategy_name = "syntax_sorcerer_the_word_order_wizard"
    ...
    def _load_rounds(self):
        """Load round metadata from assets directory."""
        # Count available rounds by checking for round_0_top.json files
        root = self._assets_path
        count = 0
        while os.path.exists(os.path.join(root, f"round_{count}_top.json")):
            self.rounds.append({"index": count})
            count += 1
